...
asset's `sha256`, and so does a Scoop manifest. `cd.yml` adopts an existing release
...
optional there. [mfeminer/scoop-bucket](https://github.com/mfeminer/scoop-bucket)'s
README says the same thing where somebody looking at a manifest will read it.
...
### Distribution — Scoop on Windows, and only Scoop

`scoop install mfeminer/lane` is the Windows half of "easy to install", and Scoop is the
only Windows package manager lane targets. The manifest lives in
[mfeminer/scoop-bucket](https://github.com/mfeminer/scoop-bucket) — a general-purpose
bucket rather than a repository about lane, the same shape as
[mfeminer/homebrew-tap](https://github.com/mfeminer/homebrew-tap) on the other side.

**This was winget first, and the reason it is not is worth keeping.** winget's pitch is
real: it ships with Windows 10 and 11, so a user installs nothing before the install
command works, where Scoop has to be installed first. What that pitch costs is the
catalogue it buys you into — a manifest is a pull request against
`microsoft/winget-pkgs`, reviewed by people who are not us, and **every release needs a
new one submitted and waited on**. Scoop inverts that: we own the bucket, so a release
needs nothing submitted anywhere. The one-line install of Scoop itself is a cost paid
once by the user; the submission was a cost paid forever by us, per release.

**Publishing is not a step, and that is the point.** Each manifest carries a `checkver`
that reads lane's GitHub releases and an `autoupdate` that rewrites the download URL for
whatever it finds. A scheduled Excavator workflow in the bucket runs Scoop's own update
tooling every four hours and commits the result. It needs no token for this repository
and this repository needs none for it — the automation lives entirely on the bucket
side, which is why adding it did not mean giving `cd.yml` a write credential for
somebody else's repo.

**The hash is computed, not copied.** No `autoupdate.hash` is configured, deliberately:
with none, Scoop downloads the new asset and hashes it itself. Reading a hash out of a
checksums file would only prove two of our own artefacts agree. And the manifest pins a
specific release asset rather than a moving `releases/latest/download/...` pointer, for
the same reason the Homebrew formula does — a moving pointer has no checksum that can
stay true.

**There is deliberately no Chocolatey package and no winget manifest.** If either is
ever wanted it is its own decision on its own merits, not something a Scoop change
quietly also does.
...
**One thing still has to be done by hand after a release**, and it is not `cd.yml`'s to
automate without giving it a write token for somebody else's repository: bump the
Homebrew formula's `url` and `sha256` in
[mfeminer/homebrew-tap](https://github.com/mfeminer/homebrew-tap). The Windows side used
to be a second such chore and is not any more — the Scoop manifest in
[mfeminer/scoop-bucket](https://github.com/mfeminer/scoop-bucket) updates itself from a
scheduled job in that repository. Both still pin a `sha256`, and both still freeze that
release's asset once published.
...
**A tag a published Homebrew formula or Scoop manifest points at is frozen.** Do not
...
[mfeminer/scoop-bucket](https://github.com/mfeminer/scoop-bucket) — a genuine need to
rebuild becomes a new patch tag instead. On the Scoop side the hash lands there on its
own within hours of the release, so the window in which a re-run is harmless is shorter
than it looks. The
